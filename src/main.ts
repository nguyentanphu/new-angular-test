...
class CleanPipe implements PipeTransform {
  transform(value: string, badWords: string) {
    const regex = new RegExp(badWords.replace(',', '|'), 'g');
  ...
  template: `
    <div class="card card-block">
      ...
      <p class="card-text" [hidden]="data.hide">
        {{ data.punchline | clean: 'boo,damn,hell' }}
      </p>
